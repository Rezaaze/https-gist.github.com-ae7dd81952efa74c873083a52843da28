...
const validateCred = (arr) => {
    ...
    lastElement = newArr.pop();
    newArr.reverse();
    let checkArr = [];
    for (let i = 0; i < newArr.length; i++){
      if(i%2 === 0){
        if(newArr[i]*2 > 9){
          checkArr.push((newArr[i]*2)-9);
        }else{
    ...
    checkArr.push(lastElement)
    const red = (a,b) => a+b;
    const total = checkArr.reduce(red);
    return total%10 === 0 ? true : false
}

const findInvalidCards = nestedArr =>{
  const newNestedArr = nestedArr.filter(arr=>!validateCred(arr));
  return newNestedArr
}
...
const idInvalidCardCompanies = nestedArr =>{
  ...
  for (let i = 0; i < nestedArr.length; i++){
    if (nestedArr[i][0] === 3){
      if(companyArr.indexOf('Amex (American Express)')=== -1){
